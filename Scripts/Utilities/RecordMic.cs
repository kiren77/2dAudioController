...
public class RecordMic : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //create a DSP object for the pitch shift effect. You can do this in the Start method after the sound object is created:
        FMOD.DSP dsp;
        RuntimeManager.CoreSystem.createDSPByType(FMOD.DSP_TYPE.PITCHSHIFT, out dsp);

//set the pitch value using the setParameterFloat method:
        float pitch = 2.0f; // Set the desired pitch value here
        dsp.setParameterFloat((int)FMOD.DSP_PITCHSHIFT.PITCH, pitch);

//After setting the pitch value, you’ll need to add the DSP object to the channel. You can do this in the Wait coroutine after the sound is played:
